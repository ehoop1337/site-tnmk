...
// Костыль для IE 9+ с динамическим CSS
//==============
...
//==============	
	...
	function b2b_resize_width(){
		return window.innerWidth+'/'+window.innerHeight+'px <br/> Ratio:'+window.devicePixelRatio+' <br/> Масштаб: '+(window.outerWidth/window.innerWidth*100+'%')
	}
	$("form").prepend('<input type="hidden" name="size_viewport" value="'+b2b_resize_width()+'">');
	window.onresize = function(){
		$("form input[name=size_viewport]").val(b2b_resize_width());
	}
	...
	$("form").prepend('<input type="hidden" name="browser" value="'+navigator.sayswho+'">');
...
//==============	
	...
	var popupvideo = get_cookie ( "browser-close" );

	if(!popupvideo){
		if(navigator.sayswho.indexOf('IE') != -1){
			$("#browser_support").addClass('active');
			setTimeout(function(){
				$("#browser_support-close").addClass('active');
			}, 10000);
			$("#browser").html(navigator.sayswho);
		}
	}
	
	
	$('#browser_support-close').on('click', function(){
		$('#browser_support').removeClass('active').addClass("hide");
		document.cookie = 'browser-close='+($('#browser_support').hasClass('active')?0:1)+'; path=/';
	});
...
!function(k){k.fn.EhoopScrollBar=function(r){var n=k.extend({runnerWidth:50,runnerHeight:6,paddingBottom:6,backgroundScroll:"rgba(0,0,0,0.3)",backgroundRunner:"#000",minWidthItem:0},r);if(null!=r){var o=n.runnerWidth,i=n.runnerHeight,e=n.paddingBottom,t=n.backgroundScroll,l=n.backgroundRunner,s=n.minWidthItem,a=0,d=(v=this).outerWidth(),c=0;v.wrapAll('<div class="wrapper-ehoopscrollbar"></div>'),(u=v.parent()).css("position","relative"),v.css({"overflow-x":"scroll","overflow-y":"hidden",display:"flex","flex-direction":"row","flex-wrap":"nowrap",position:"relative",padding:"0px 0px "+e+"px 0px"}).addClass("ehoopScrollBar"),x=v.find("div").length,0<s&&v.find("div").css("min-width",s);for(var p=0;p<x;p++)p==x-1&&v.find("div:eq("+p+")").css("margin-right",0),a+=v.find("div:eq("+p+")").outerWidth(!0);function h(){m<c+3&&0<m&&v.find(".ehoopscrollbar").css({transform:"translateX("+m+"px)"})}function f(){parseInt(m/w),b.css("transform","translateX("+(d/100*W-o*W/100)+"px)")}u.append('<div class="ehoopscrollbar"><span></span></div>'),g=u.find(".ehoopscrollbar"),k(g).css({position:"absolute",left:0,right:0,bottom:0,height:i+"px",width:d+"px",background:t,overflow:"hidden",border:"none",margin:0}),b=g.find("span"),k(b).css({position:"absolute",left:"0%","top:":0,width:o+"px",height:i+"px",background:l}),w=(c=a-d)/100,v.scroll(function(){m=k(this).scrollLeft(),h(),f()})}else{var u,v,g,b,x,m,w,W,a=0,d=(v=this).outerWidth(),c=0;if(v.wrapAll('<div class="wrapper-ehoopscrollbar"></div>'),u=v.parent(),v.addClass("inner-ehoopscrollbar"),0<v.children("div").length){x=v.children("div").length;for(p=0;p<x;p++)a+=v.children("div:eq("+p+")").outerWidth(!0)}else if(0<v.children("a").length){x=v.children("a").length;for(p=0;p<x;p++)a+=v.children("a:eq("+p+")").outerWidth(!0)}function h(){m<c+3&&0<m&&v.find(".ehoopscrollbar").css({transform:"translateX("+m+"px)"})}function f(){W=m/w,b.css("transform","translateX("+(d/100*W-k(b).outerWidth()*W/100)+"px)")}u.append('<div class="ehoopscrollbar"><span></span></div>'),g=u.find(".ehoopscrollbar"),b=g.find("span"),w=(c=a-d)/100,v.scroll(function(){m=k(this).scrollLeft(),h(),f()})}}}(jQuery);